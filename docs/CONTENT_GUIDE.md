...
v3では20教材を `dist/src/lessons/basic.js`、`sensors.js`、`extended.js` に分けて収録しています。`dist/src/content.js` が配列を統合します。ここへ同じ形式の教材を追加すると、教材選択に表示されます。各教材の作業状態は端末内で個別に保存されます。
